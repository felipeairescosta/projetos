...
grau = st.sidebar.radio(
    "Grau de jurisdição",
    options=["1º Grau", "2º Grau"],
    index=0,
)

df = df_1g if grau == "1º Grau" else df_2g
...
# Filtro de órgão julgador (só quando ano específico selecionado)
orgao_selecionado = "Todos"
if ano_selecionado != "Todos":
    orgaos_disponiveis = sorted(df_filtrado["orgao"].dropna().unique().tolist())
...
if grau == "2º Grau":
    st.sidebar.caption("ℹ️ Processos de Corregedoria excluídos")
...
# Segunda linha de gráficos (só para 2G)
if grau == "2º Grau":
    col_esq, col_dir = st.columns(2)
